...
(() => {
  ...
  function renderPillarList() {
    ...
    if (!pillars.length) {
      list.innerHTML = `<div style="padding:20px;text-align:center;color:var(--text-dim);font-family:var(--font-mono);font-size:11px;letter-spacing:2px;">NO PILLARS YET</div>`;
      return;
    }
    ...
    list.querySelectorAll('.delete-pillar').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const taskCount = state.tasks.filter(t => t.tag === pillar.id).length;
        const msg = taskCount > 0
          ? `Delete "${pillar.name}"? ${taskCount} task(s) will move to OTHER.`
          : `Delete "${pillar.name}"?`;
        forgeConfirm(msg, () => {
